...
class Tello:
    ...
    def __init__(self, ips: tuple[int | None] = ('192.168.10.1', '0.0.0.0'), ports: tuple[int | None] = (8889, 8890, 11111), **preferences) -> None:
        if type(ips) is not tuple or len(ips) != 2: raise ValueError('IP addresses provided were invalid. Please make sure it\'s a valid tuple with at least 1 address with others inputted as None')
        elif type(ports) is not tuple or len(ports) != 3: raise ValueError('Ports provided were invalid. Please make sure it\'s a valid tuple with at least 1 port with others inputted as None')
        ...
        for ip in ips:
            if ips.index(ip) == 1 and type(ip) is None: raise ValueError('IP addresses provided were invalid. Please make sure it\'s a valid tuple with at least 1 address with others inputted as None')
            if ip: ip_address(ip)
    ...
    def set_ports(self, ports: tuple[Union[int, None]]) -> None:
        ...
        for port in ports:
            if ports.index(port) == 1 and type(port) is None: raise ValueError('Ports provided were invalid. Please make sure it\'s a valid tuple with at least 1 port with others inputted as None')
