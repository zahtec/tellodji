...
class Tello:
    ...
    def __init__(self, ips: tuple[int | None] = ('192.168.10.1', '0.0.0.0'), ports: tuple[int | None] = (8889, 8890, 11111), **preferences) -> None:
        if type(ips) is not tuple or len(ips) != 2 or (ips[0] and ips[1] == None): raise ValueError('IP addresses provided were invalid. Please make sure it\'s a valid tuple with at least 1 address with others inputted as None')
        elif type(ports) is not tuple or len(ports) != 3 or (ports[0] and ports[1] and ports[2] == None): raise ValueError('Ports provided were invalid. Please make sure it\'s a valid tuple with at least 1 port with others inputted as None')
        ...
        elif 'mission_pad' in preferences and type(preferences['mission_pad']) is not bool: raise ValueError('Mission pad preference provided was invalid. Please make sure it\'s a valid boolean type')
        elif 'safety' in preferences and type(preferences['safety']) is not bool: raise ValueError('Safety preference provided was invalid. Please make sure it\'s a valid boolean type')
        elif 'sync' in preferences and not (type(preferences['sync']) is bool or callable(preferences['sync'])): raise ValueError('Sync preference provided was invalid. Please make sure it\'s a valid boolean type or a callable')
        elif 'takeoff' in preferences and type(preferences['takeoff']) is not bool: raise ValueError('Takeoff preference provided was invalid. Please make sure it\'s a valid boolean type')
        elif 'syncfix' in preferences and type(preferences['syncfix']) is not bool: raise ValueError('Syncfix preference provided was invalid. Please make sure it\'s a valid boolean type')
        elif 'debug' in preferences and type(preferences['debug']) is not bool: raise ValueError('Debug preference provided was invalid. Please make sure it\'s a valid boolean type')
        elif 'video' in preferences and type(preferences['video']) is not bool: raise ValueError('Video preference provided was invalid. Please make sure it\'s a valid boolean type')

        for ip in ips:
            if type(ip) is not str or None: raise ValueError('One or more IP\'s provided were invalid. Please make sure they\'re valid strings and in proper IP address form')
            if ip: ip_address(ip)

        for port in ports:
            if port and type(port) is not int: raise ValueError('One or more ports provided were invalid. Please make sure they are integers and in proper port form')
        ...
        if 'video' in preferences and preferences['video']: self._send_sync('streamon', 'basic')
    ...
    def set_ips(self, ips: tuple[str | None]) -> None:
        '''
    ...
    def set_ports(self, ports: tuple[int | None]) -> None:
        '''
        ...
        if type(ports) is not tuple or len(ports) != 3 or (ports[0] and ports[1] and ports[2] == None): raise ValueError('Ports provided were invalid. Please make sure it\'s a valid tuple with at least 1 port with others inputted as None')

        for port in ports:
            if type(port) is not int or None: raise ValueError('One or more ports provided were invalid. Please make sure they\'re valid integers and in proper port form')
    ...
    def set_debug(self, preference: bool) -> None:
        ...
        if type(preference) is not bool: raise ValueError('Preference value provided was invalid. Please make sure it\'s a valid boolean type')
    ...
    def set_default_distance(self, distance: int) -> None:
        ...
        if type(distance) is not int or not 20 <= distance: raise ValueError('Distance value provided was invalid. Please make sure it is a valid integer and at least 20cm')
    ...
    def set_default_rotation(self, rotation: int) -> None:
        ...
        if type(rotation) is not int or not 10 <= rotation <= 360: raise ValueError('Rotation value provided was invalid. Please make sure it\'s a valid integer and in between 1 - 360deg')
    ...
    def set_default_speed(self, speed: int) -> None:
        ...
        if type(speed) is not int or not 10 <= speed <= 60: raise ValueError('Speed value provided was invalid. Please make sure it\'s a valid integer and between 10 - 60cm/s')
    ...
    def set_timeout(self, timeout: int) -> None:
        ...
        if type(timeout) is not int or timeout < 1: raise ValueError('Timeout provided was invalid. Please make sure it\'s a integer and at least 1 second')
    ...
    def _send_sync(self, msg: str, typ: str, val: int | tuple[Union[int, str]] = None) -> str:
        ...
        match typ:
            ...
            case 'dist':
                if type(val) is not int or not 20 <= val: raise ValueError('Distance value is incorrect. Please make sure it\'s a valid integer and at least 20cm')
            ...
            case 'rot':
                if type(val) is not int or not 1 <= val <= 360: raise ValueError('Rotational value is incorrect. Please make sure it\'s a valid integer and between 1 - 360deg')
            ...
            case 'setspd':
                if type(val) is not int or not 10 <= val <= 60: raise ValueError('Speed value is incorrect. Please make sure it\'s a valid integer and between 10 - 60cm/s')
            ...
            case 'setrc':
                ...
                for n in val:
                    if type(n) is not int or not -100 <= n <= 100: raise ValueError('Joystick distance value is incorrect. Please make sure it\'s a valid integer and between -100 - 100')
                    msg += f'{n} '
            ...
            case 'setwifi':
                ...
                if type(val[0]) is not str or not len(val[0]) or not val[0].isascii(): raise ValueError('WiFi SSID value is incorrect. Please make sure it\'s a valid string in ascii and at least 1 character')
                elif type(val[1]) is not str or not len(val[1]) >= 5 or not val[1].isascii(): raise ValueError('WiFi password value is incorrect. Please make sure it\'s a valid string in ascii and at least 5 characters')
            ...
            case 'connwifi':
                ...
                if type(val[0]) is not str or not len(val[0]): raise ValueError('WiFi SSID value is incorrect. Please make sure it\'s a valid string and at least 1 character')
                elif type(val[1]) is not str or not len(val[1]): raise ValueError('WiFi password value is incorrect. Please make sure it\'s a valid string and at least 1 character')
            ...
            case 'mpad':
                ...
                elif val is not int or not 0 <= val <= 2: raise ValueError('Mission Pad Detection value is incorrect. Please make sure it\'s a valid integer and between 0 - 2')
        ...
        match typ:
            ...
            case 'mid':
                ...
                for n in val:
                    ...
                    elif ind == leg-1 and type(n) is not str or not match(r'^[m][1-8]$', val): raise ValueError('Mission Pad value is incorrect. Please make sure it\'s a valid string and is between m1 - m8')
                    elif not isd or not -500 <= n <= 500: raise ValueError('coordinate value is incorrect. Please make sure it\'s a valid integer and between -500 - 500cm')
            ...
            case 'jump':
                ...
                for n in val:
                    ...
                    elif ind == leg-1 or ind == leg-2 and type(n) is not str or not match(r'^[m][1-8]$', val): raise ValueError('Mission Pad value is incorrect. Please make sure it\'s a valid string and is between m1 - m8')
                    elif not isd or not -500 <= n <= 500: raise ValueError('coordinate value is incorrect. Please make sure it\'s a valid integer and between -500 - 500cm')
            ...
            case 'setrc':
                ...
                for n in val:
                    if type(n) is not int or not -100 <= val <= 100: raise ValueError('Joystick distance value is incorrect. Please make sure it\'s a valid integer and between -100 - 100')
                    msg += f'{n} '
            ...
            case 'setwifi':
                ...
                if type(val[0]) is not str or not len(val[0]): raise ValueError('WiFi SSID value is incorrect. Please make sure it\'s a valid string and at least 1 character')
                elif type(val[1]) is not str or not len(val[1]) >= 5: raise ValueError('WiFi password value is incorrect. Please make sure it\'s a valid string and at least 5 characters')
                elif not match(r'(?=(?:[^a-z]*[a-z]){2})(?=(?:[^A-Z]*[A-Z]){2})(?=(?:[^0-9]*[0-9]){1})', val[1]): raise ValueError('WiFi password value is insecure. Please make sure it contains at least 2 lowercase and uppercase letters and 1 number')
            ...
            case 'mpad':
                if not self._mp: raise ValueError('Mission pad detection hasn\'t been enabled yet. Please run the Tello.set_mission_pad() method first')
                elif type(val) is not int or not 0 <= val <= 2: raise ValueError('Mission Pad Detection value is incorrect. Please make sure it\'s a valid integer and between 0 - 2')
    ...
    def get_speed(self, **preferences) -> Union['Tello', str]:
        ...
        if 'callback' in preferences and preferences['callback'] != False and not callable(preferences['callback']): raise ValueError('Callback function is incorrect. Please make sure it\'s a callable')
    ...
    def get_mpxyz(self) -> tuple[str] | None:
        '''
    ...
    def photo(self, **preferences) -> Union['Tello', str]:
        ...
        if type(window) is not bool: raise ValueError('Window preference provided was invalid. Please make sure it\'s a valid boolean type')
        ...
        elif not (callback == False or None) and not callable(callback): raise ValueError('Callback function is incorrect. Please make sure it\'s a callable')

        for res in resolution:
            if type(res) is not int: raise ValueError('Provided resolution was invalid. Please make sure it\'s a valid tuple with 2 integers providing the width and height of the resolution')
    ...
    def photo_bytes(self, **preferences) -> Union['Tello', bytes]:
        ...
        elif callback != False and not callable(callback): raise ValueError('Callback function is incorrect. Please make sure it\'s a callable')
    ...
    def start_video(self, **preferences) -> None:
        ...
        elif callback != None and not callable(callback): raise ValueError('Callback function is incorrect. Please make sure it\'s a callable')
    ...
    def video_bytes(self, **preferences) -> Union['Tello', list[bytes]]:
        ...
        if type(resolution) is not tuple or len(resolution) != 2: raise ValueError('Provided resolution was invalid. Please make sure it\'s a valid tuple with 2 integers providing the width and height of the resolution')
        elif type(frames) is not int: raise ValueError('Provided frame count was invalid. Please make sure it\'s a valid integer')
        elif callback != False and not callable(callback): raise ValueError('Callback function is incorrect. Please make sure it\'s a callable')
